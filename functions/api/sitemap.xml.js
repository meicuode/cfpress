 ...
 * GET /api/sitemap.xml
 *
 * 使用 Cloudflare Cache API 在边缘节点缓存 1 小时
 */

import { withCache } from './_utils/cache.js';
...
async function handleGet(context) {
  ...
  try {
    const xml = await generateSitemap(env, request);
...
export const onRequestGet = withCache(handleGet, { ttl: CACHE_TTL });
